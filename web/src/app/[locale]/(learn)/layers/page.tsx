    ...
    <div className="py-4">
      ...
      <div className="space-y-6">
        {LAYERS.map((layer, index) => {
          const lessonInfos = layer.lessons.map((lId) => {
            const info = data.versions.find((v) => v.id === lId);
            const meta = LESSON_META[lId];
            return { id: lId, info, meta };
          });
                        ...
                        <div className="flex items-start justify-between">
                          <div className="min-w-0 flex-1">
                            ...
                            <h3 className="mt-1 font-semibold text-zinc-900 dark:text-zinc-100">
                              {locale === "zh" ? meta?.titleZh : meta?.title || id}
                            </h3>
                            {meta && (
                              <p className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                                {locale === "zh" ? meta.subtitleZh : meta.subtitle}
                              </p>
                            )}
                          </div>
                          <ChevronRight
