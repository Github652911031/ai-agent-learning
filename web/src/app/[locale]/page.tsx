      ...
      <section>
        ...
        <div className="mx-auto max-w-2xl overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950">
          <div className="flex items-center gap-2 border-b border-zinc-800 px-4 py-2.5">
            ...
            <span className="ml-3 text-xs text-zinc-500">AgentLoop.java</span>
          </div>
          <pre className="overflow-x-auto p-4 text-sm leading-relaxed">
            <code>
              ...
              <span className="text-zinc-300">) {"{"}</span>
              {"\n"}
              <span className="text-zinc-300">{"    "}response = client.chat().completions().</span>
              ...
              <span className="text-blue-400">isEmpty</span>
              <span className="text-zinc-500">())</span>
              {"\n"}
              ...
              <span className="text-zinc-300"> (toolCall : response.toolCalls()) {"{"}</span>
              {"\n"}
              ...
              <span className="text-zinc-500">,</span>
              <span className="text-zinc-300"> toolCall.arguments()</span>
              <span className="text-zinc-500">);</span>
              {"\n"}
              ...
              >
                ...
                >
                  ...
                  <h3 className="mt-3 text-sm font-semibold group-hover:underline">
                    {locale === "zh" ? meta.titleZh : meta.title}
                  </h3>
                  <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
                    {locale === "zh" ? meta.keyInsightZh : meta.keyInsight}
                  </p>
                </Card>
              </Link>
            );
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[var(--color-text-secondary)]">
                    {layer.lessons.length} {t("lessons_in_layer")}
                  </span>
                </div>
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {layer.lessons.map((lId) => {
                    ...
                    return (
                      <Link key={lId} href={`/${locale}/${lId}`}>
                        <LayerBadge
                          layer={layer.id}
                          className="cursor-pointer transition-opacity hover:opacity-80"
                        >
                          {lId}: {locale === "zh" ? meta?.titleZh : meta?.title}
                        </LayerBadge>
                      </Link>
                    );
                  })}
